...
function SubjectRow({
  ...
  return (
    <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-primary-100 bg-white px-2.5 py-2 hover:bg-primary-50">
      <input
        type="checkbox"
        checked={checked}
        onChange={onToggle}
  ...
  return (
    <div className="grid auto-cols-[240px] grid-flow-col gap-3 overflow-x-auto pb-2">
      {years.map(([year, { Fall, Spring }]) => {
        ...
        const allSelected = yearCodes.length > 0 && yearCodes.every((code) => selected.has(code));

        return (
          <div key={year} className="flex flex-col gap-2">
            <div className="rounded-lg bg-primary-800 px-3 py-2 text-center text-sm font-semibold text-white">
              السنة {YEAR_LABELS_AR[year - 1] ?? year}
            </div>
            <label className="flex cursor-pointer items-center justify-center gap-1.5 rounded-md border border-primary-200 bg-primary-50 px-2 py-1.5 text-[11px] font-medium text-primary-700 hover:bg-primary-100">
              <input
                type="checkbox"
                checked={allSelected}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-primary-400">
          اخترت {selected.size} مادة. لو لسه ما خلصتش أي مادة، كمّل من غير ما تختار حاجة.
        </p>
